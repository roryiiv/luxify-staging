...
class Panel extends Controller
{
        ...
        if(isset($_POST['profile_img']) && !empty($_POST['profile_img'])){
            ...
            if (file_exists($image) ) {
              if($s3->put($filePath, file_get_contents($image), 'public')){
                  $input->mainImageURL = $_POST['profile_img'];
                  unlink($image);
              }
            }
        }
    ...
    public function category_update(){
        ...
        if(isset($_POST['profile_img']) && !empty($_POST['profile_img'])){
            ...
            if (file_exists($image) ) {
              if($s3->put($filePath, file_get_contents($image), 'public')){
                  $update->mainImageURL = $_POST['profile_img'];
                  unlink($image);
              }
            }
        }
...
/*        if(isset($_POST['slug']) && !empty($_POST['slug'])){
    ...
    public function user_delete($user_id) {
        ...
        DB::table('users')
        ->where('id', $user->id)
        ->update(['isSuspended' => 1, 'dealer_status' => 'rejected']);
    ...
    public function user_revoke($user_id) {
        ...
        DB::table('users')
        ->where('id', $user->id)
        ->update(['isSuspended' => 0, 'dealer_status' => 'approved']);
