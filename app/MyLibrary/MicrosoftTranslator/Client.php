...
class Client
{
	...
	const API_CLIENT_SCOPE = 'http://api.microsofttranslator.com';
	const AUTH_BASE_URL    = 'https://datamarket.accesscontrol.windows.net/v2/OAuth2-13';
	...
	public function translate( $text , $to , $from = null , $contentType = 'text/plain' , $category = 'general' )
	{
		...
		return $this->get( '/Translate' , array() , $query_parameters );
	}
	...
	public function breakSentences( $text , $from )
	{
		...
		return $this->get( '/BreakSentences' , array() , $query_parameters );
	}
	...
	public function TransformText( $text , $from , $category = 'general' )
	{
		...
		return $this->get( '/TransformText' , array() , $query_parameters , true , 'http://api.microsofttranslator.com/V3/json/' );
	}
	...
	public function translateArray( array $texts , $to , $from = null , $contentType = 'text/plain' , $category = 'general' )
	{
		...
		return $this->post( '/TranslateArray' , array() , $requestXml , true , $texts );
	}
	...
	public function getLanguageNames( $locale , $languageCodes )
	{
		...
		return $this->post( '/GetLanguageNames?locale=' . $locale , array() , $requestXml , true , $languageCodes );
	}
	...
	public function detect( $text )
	{
		...
		return $this->get( '/Detect' , array() , $query_parameters );
	}
	...
	public function detectArray( array $texts )
	{
		...
		return $this->post( '/DetectArray' , array() , $requestXml , true , $texts );
	}
	 ...
	 * @return string
	 */
	private function buildUrl( $endpoint , $url_parameters = array() , $special_url = null )
	{
	...
	private function getResponseObject( $endpoint , $url_parameters , $query_parameters , $try_to_auth , $url , $result , $originalData = null )
	{
		...
		if ( isset( $result[ 'http_body' ] ) )
		{
			...
			else if ( Tools::startsWith( $result[ 'http_body' ] , '<ArrayOfTranslateArrayResponse ' ) )
			{
				$xmlObj = simplexml_load_string( $result[ 'http_body' ] );
				$array  = array();
				$i      = 0;
			...
			else if ( Tools::startsWith( $result[ 'http_body' ] , '<ArrayOfstring ' ) )
			{
				$xmlObj = simplexml_load_string( $result[ 'http_body' ] );
				$array  = array();
				$i      = 0;
			...
			else if ( Tools::startsWith( $result[ 'http_body' ] , '<ArrayOfint ' ) )
			{
				$xmlObj   = simplexml_load_string( $result[ 'http_body' ] );
				$array    = array();
				$i        = 1;
			...
			else if ( ! is_null( $json = json_decode( Tools::removeUtf8Bom( $result[ 'http_body' ] ) , true ) ) )
			{
	 ...
	 * @throws \MicrosoftTranslator\Exception
	 */
	private function get( $endpoint , $url_parameters = array() , $query_parameters = array() , $try_to_auth = true , $special_url = null )
	{
		...
		return $this->getResponseObject(
			...
			$try_to_auth ,
			$url ,
			$this->http->get( $url , $api_access_token , $query_parameters )
		);
	}
	 ...
	 * @throws \MicrosoftTranslator\Exception
	 */
	private function post( $endpoint , $url_parameters = array() , $query_parameters = array() , $try_to_auth = true , $originalData = null )
	{
		...
		return $this->getResponseObject(
			...
			$try_to_auth ,
			$url ,
			$this->http->post( $url , $api_access_token , $query_parameters ) ,
			$originalData
		);
	}
	 ...
	 * @codeCoverageIgnore
	 */
	private function put( $endpoint , $url_parameters = array() , $query_parameters = array() , $try_to_auth = true , $originalData = null )
	{
		$url              = $this->buildUrl( $endpoint , $url_parameters );
		$api_access_token = ( $try_to_auth === true ) ? ( is_null( $this->api_access_token ) ) ? $this->auth->getAccessToken() : $this->api_access_token : '';
		$query_parameters = $this->fixQueryParameters( $query_parameters );
		...
		return $this->getResponseObject(
			...
			$try_to_auth ,
			$url ,
			$this->http->put( $url , $api_access_token , $query_parameters ) ,
			$originalData
		);
	}
	...
	private function delete( $endpoint , $url_parameters = array() , $query_parameters = array() , $try_to_auth = true , $originalData = null )
	{
		...
		return $this->getResponseObject(
			...
			$try_to_auth ,
			$url ,
			$this->http->delete( $url , $api_access_token , $query_parameters ) ,
			$originalData
		);
	}
